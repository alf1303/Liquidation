...
class Results():
    ...
    def __str__(self):
        ...
        res += f"YES votes: *{self.votes_for} ({round(self.voted_for/10**8, 2)} sPLUTO, {round(self.voted_for*100/(self.voted_for + self.voted_against), 2)}% of voted)*\n"
        res += f"NO votes: *{self.votes_against} ({round(self.voted_against/10**8, 2)} sPLUTO, {round(self.voted_against*100/(self.voted_for + self.voted_against), 2)}% of voted)*\n"
        res += f"TOTAL votes: *{self.votes_for + self.votes_against} ({round(self.voted_staked/10**8, 2)} sPLUTO, {perc}% of staked)*"
        return res
